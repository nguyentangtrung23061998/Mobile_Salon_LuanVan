...
import {Button, Header, Input as RNEInput} from 'react-native-elements';
...
import useCustomerList0 from '../customer_list0/hook';
import useCustomerList1 from '../customer_list1/hook';
...
export default React.memo(() => {
  ...
  const {onGetAllCustomersEvent0, onResetStateEvent0} = useCustomerList0();
  const {onGetAllCustomersEvent} = useCustomerList1();
  const initialValues = {
    fullname: '',
    mobile: '',
  };
  ...
  return (
    <Container>
      ...
      <MyScrollView0 contentContainerStyle={styles.myScrollView00}>
        ...
        <RNEInput
          errorMessage={
            errors.mobile && !state.isMobileFocused && !state.isMobileValid
              ? errors.mobile
              : ''
          }
